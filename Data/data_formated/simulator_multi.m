%This file is used to simulate the modified particle filter using the gathered data.
%It is largely uncommented and needs significant restructuring. This
%restructuring will not change the functionality. Note, it this file shares
%many similarties with simulator_regpf_multi.
...
for(zz=1:length(trail_list))
    ...
    % into temporary storage. Use the mean robot position and ranges to
    % update the landmark positions. Then use the mean landmark positions
    % to update the robots position.
    ...
    for(ii=2:length(turtlebot_odom_command)-1)
        ...
        if(exist('landmarkFilters') && ranges_aval)
            for(jj=1:length(pozyx_anchor_names))
                VARCONST=.40;
                if((var(landmarkFilters{jj}.particles(:,1))<VARCONST) &&( var(landmarkFilters{jj}.particles(:,2))<VARCONST)&& (turtlebot_odom_command_timestamps(ii)>10))
