classdef pozyx_tag < handle
    ...
    properties
        ...
        distanceMeasure % Array of distance measurments (in m) stored in cell array (1xn)
        numMeasurments; % The number of measurments received from each sensor (1xn)
        sampleRate;     % The rate at which each anchor is sampled in Hz (1xn) (NOT IMPLEMENTED)
        anchorTimers;   % Used to automatically poll each sensor at sampleRate (1xn)
        tagID;          % The hex identification of the tag
        topicName;      % The name of the pozyx topic to subscribe to.
        pozyxSubsriber  % The subscriber subscribed to the pozyx measurment topic.
        startTime       % The time that the first message is received 
        youngDistanceMeasure % A list of distance measurments (in m) that were received within timeLimit seconds 
        sensorHeight %The height of the sensor relative to the turtlebot reciever
        
        % NOTE: The coords of the pozyx anchors are not stored in this
        % class
    end
    ...
    methods
        function obj = pozyx_tag(tagID,anchorIDs,varargin)
            ...
            obj.sensorHeight=1.3081-0.4191;
